...
import os
import requests
from typing import Annotated, Literal, Any, Dict, List
...
IMPORTANT: For queries related to MFA (multi-factor authentication) or security, the Users table is most relevant.
...
def debug_state(state_dict, node_name=None):
    ...
    if "error" in state_dict:
        print(f"Error: {state_dict['error']}")
...
# --- Utility Functions ---
def handle_tool_error(state: Any) -> Dict[str, List[ToolMessage]]:
...
# --- Step 1: List Tables ---
def list_tables_node(state_dict) -> Dict[str, List[AIMessage]]:
    ...
    result = {
        "messages": [
            AIMessage(
                content="",
                tool_calls=[{"name": "sql_db_list_tables", "args": {}, "id": "list_tables_call_id"}],
            )
        ]
    }
...
def get_schema_node(sstate_dict) -> Dict[str, List[AIMessage]]:
    ...
    tables = ""
    for msg in reversed(sstate_dict["messages"]):
        if isinstance(msg, ToolMessage) and msg.tool_call_id == "list_tables_call_id":
            tables = msg.content
            break
...
def query_gen_node(state_dict) -> Dict[str, List[AIMessage]]:
    ...
    try:
        ...
        if has_schema:
            ...
            response = query_gen.invoke(state_dict)
        ...
        if has_results:
            ...
            response = llm_with_tools.invoke(
                query_gen_prompt.format(messages=state_dict["messages"])
            )
            
            result = {"messages": [response]}
            return debug_state(result, "query_gen_node - final answer")
        
        # Default case - just pass through the model's output
        response = query_gen.invoke(state_dict)
        result = {"messages": [response]}
...
def check_query_node(state_dict) -> Dict[str, List[AIMessage]]:
    """Check and validate SQL queries before execution."""
    print("\nRunning check_query_node...")
    
    # Find the last message with a query
    query = ""
    tool_call_id = ""
    for msg in reversed(state_dict["messages"]):
    ...
    if not query:
        ...
        return {"messages": state_dict["messages"][-1:]}
    ...
    try:
        ...
        checked_query = check_result.content
...
def should_continue(state: State) -> Literal[END, "check_query", "query_gen"]:
    ...
    if hasattr(last, "tool_calls") and any(tc.get("name") == "db_query_tool" for tc in last.tool_calls):
...
if __name__ == "__main__":
    ...
    question = "Which users has mfa none?"
